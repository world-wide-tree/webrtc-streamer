...
async fn main() -> Result<(), Box<dyn std::error::Error>>{
    ...
    // send offer to SignalService...
    let r = clnt.post("http://0.0.0.0:3030/offer/0").body(serde_json::to_string(&offer)?).send().await?;
    
    // Set local description...
    peer_connection.set_local_description(offer).await?;
    
    // await answer from client
    let answer = loop {
    ...
    loop {
        if let Ok(frm_buf) = tr.try_recv(){
            ...
            match dc.ready_state(){
                ...
                RTCDataChannelState::Closed => break,
                _ => continue,
            }
        }
